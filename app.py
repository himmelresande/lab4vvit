...
def registration():
    if request.method == 'POST':
        ...
        if len(check_login) > 0:
            return render_template('registration.html', error='Login can not consist only of numbers and can not contain special characters and spaces')

        if ' ' in reg_password:
            return render_template('registration.html', error='Password can not contain spaces')
